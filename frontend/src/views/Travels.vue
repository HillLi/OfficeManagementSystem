  ...
  <div class="page-grid">
    <div class="panel">
      ...
      <el-form label-position="top">
        ...
        <el-form-item label="人员类别">
          <el-select v-model="form.staffLevel">
        ...
        <el-form-item label="出差类型">
          <el-select v-model="form.travelType">
    ...
    <div class="panel">
      ...
      <el-table :data="rows" border>
        ...
        <el-table-column prop="actualExpense" label="实报" width="100" />
        ...
        <el-table-column label="操作" width="120">
          <template #default="{ row }">
            <el-button v-if="row.status === 'approved'" size="small" type="success" @click="reimburse(row)">报销登记</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
const reimburse = async (row) => {
  const { value } = await ElMessageBox.prompt('请输入实际报销金额', '报销登记', {
    inputValue: row.actualExpense || row.budget || 0
  })
  await api.reimburseTravel(row.id, { actualExpense: Number(value), receiptUrl: `/uploads/travel-${row.id}.pdf` })
  ElMessage.success('报销申请已提交财务')
  load()
}
onMounted(load)
</script>
