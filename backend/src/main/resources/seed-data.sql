-- Seed data: auto-executed on every backend startup.
-- All statements are idempotent (INSERT IGNORE / ON DUPLICATE KEY UPDATE),
-- safe to run repeatedly. Add new dictionary items here — no Java change needed.
